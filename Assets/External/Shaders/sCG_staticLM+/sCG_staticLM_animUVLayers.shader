...
Shader "socialPointCG/sCG_staticLM+/sCG_staticLM_animUVLayers" 
{
	Properties {		
		_MainTex ("DiffuseMap (ch1)", 2D) = "white" {}
		[NoScaleOffset] _MK ("MaskMap (ch1) (red channel is alpha mask)", 2D) = "red" {}
	...
	SubShader {
		...
		uniform float4		_ScrollMain;
		uniform float4		_ScrollSec;
		...
		struct vertexOutput {
			...
			half2 uv1 : TEXCOORD4;
			float2 uvMainLayer : TEXCOORD5;
			float2 uvSecondaryLayer : TEXCOORD6;
			#if USE_VC
				fixed4 VC : COLOR;
			#endif
		};
		...
		vertexOutput vert(vertexInput v)
		{
			...
			o.uvMainLayer = o.uv0 * _ScrollMain.zw + frac(_ScrollMain.xy * _Time);
			o.uvSecondaryLayer = o.uv0 * _ScrollSec.zw + frac(_ScrollSec.xy * _Time);
		...
		fixed4 frag(vertexOutput i) : COLOR
		{
			...
			fixed4 MAIN_LAYER = tex2D(_MainTex, i.uvMainLayer);
			fixed4 SEC_LAYER = tex2D(_MainTex, i.uvSecondaryLayer);
			...
			fixed3 light = LM.rgb * VC.rgb * shadow;
			...
			#if USE_SECONDARY_LAYER
				fixed4 DF = MAIN_LAYER * SEC_LAYER;
			#else
				fixed4 DF = MAIN_LAYER;
			#endif

			fixed4 Complete = fixed4(DF.rgb * _Color.rgb * light * _Overbright + emissiveContrib, VC.a * DF.a * _Color.a * MK.r);
